...
mod test_helpers {
    ...
    use crate::test_utils::{get_public_grpc_conn, run_tower_in_background_with_config, ApiConfig};
    ...
    pub(crate) async fn get_tower_message_handler_with_config(
        conf: ApiConfig,
    ) -> (Arc<TowerMessageHandler>, Arc<InternalAPI>) {
        let (server_addr, internal_api) = run_tower_in_background_with_config(conf).await;
    ...
    pub(crate) async fn get_tower_message_handler() -> (Arc<TowerMessageHandler>, Arc<InternalAPI>)
    {
    ...
    pub(crate) async fn run_lightning_tower() -> (SocketAddr, PublicKey) {
...
mod message_handler_tests {
    ...
    async fn test_register() {
        let (tower, _) = get_tower_message_handler().await;
    ...
    async fn test_add_appointment_already_triggered() {
        let (tower, internal_api) = get_tower_message_handler().await;
        let (user_sk, user_pk) = get_random_keypair();
        let msg = Register {
            pubkey: user_pk,
            // The tower doesn't use this info ATM.
...
mod peer_manager_tests {
    ...
    async fn simple_test() {
        let (tower_addr, tower_pk) = run_lightning_tower().await;
